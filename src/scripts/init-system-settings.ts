...
async function initializeSystemSettings() {
  try {
    ...
    if (existingSettings) {
      ...
      logger.info(
        `   Email Automation: ${existingSettings.emailAutomationEnabled ? "ENABLED" : "DISABLED"}`
      );
      ...
      logger.info(
        `   Email Automation: ${settings.emailAutomationEnabled ? "ENABLED" : "DISABLED"}`
      );
      logger.info(`   Created At: ${settings.createdAt}`);
      logger.info("");
      logger.info("📌 NOTE: Email automation is DISABLED by default.");
      logger.info(
        "   You can enable it from the admin panel in the Email Monitoring Settings page."
      );
    }
