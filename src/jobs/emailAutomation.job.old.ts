...
class EmailAutomationJob {
  ...
  async processEmails(): Promise<void> {
    ...
    try {
      ...
      this.stats.totalEmailsProcessed += totalProcessed;
      this.stats.totalCandidatesCreated += totalCreated;
      this.stats.totalErrors += totalErrors;
      this.stats.lastRunAt = new Date();
      this.stats.lastRunDuration = parseFloat(duration);
    ...
    try {
      ...
      for (const email of emails) {
        try {
          ...
          // Check for resume attachments (filter out obvious non-resumes)
          const nonResumeKeywords = /invoice|receipt|statement|bill|order|confirmation/i;
          const resumeAttachments = email.attachments?.filter((att: any) => {
            const isDocumentFile = /\.(pdf|doc|docx)$/i.test(att.filename);
            const isLikelyResume = !nonResumeKeywords.test(att.filename);
            return isDocumentFile && isLikelyResume;
          });
          ...
          // Parse resume with Affinda (primary) or OpenAI (fallback)
          logger.info(`Step 1: Parsing resume with ${config.affinda.useAffinda ? 'Affinda (primary)' : 'OpenAI'}...`);
          let parsedResume;
          try {
            if (config.affinda.useAffinda && config.affinda.apiToken) {
              try {
                logger.info('Using Affinda parser (primary)');
                parsedResume = await affindaService.parseResume(
                  attachment.content,
                  attachment.filename  // Affinda handles full filename
                );
                logger.info('Step 1: Resume parsed successfully with Affinda');
              } catch (affindaError: any) {
                logger.warn('Affinda parsing failed, falling back to OpenAI:', affindaError.message);
                // Fallback to OpenAI
                const fileExtension = attachment.filename.split('.').pop()?.toLowerCase() || 'pdf';
                logger.info(`File extension: ${fileExtension}`);
                parsedResume = await openaiService.parseResumeFromFile(
                  attachment.content,
                  fileExtension
                );
                logger.info('Step 1: Resume parsed successfully with OpenAI (fallback)');
              }
            } else {
              // Use OpenAI directly
              ...
              logger.info('Step 1: Resume parsed successfully with OpenAI');
            }
          } catch (parseError: any) {
            logger.error(`Step 1 FAILED: ${parseError?.message || String(parseError)}`);
            logger.error(`Parse error type: ${typeof parseError}`);
          ...
          logger.info(`Step 2: Validating resume with AI...`);
...
Education: ${parsedResume.education?.map(edu => `${edu.degree} from ${edu.institution}`).join('; ') || 'N/A'}
          ...
          logger.info(`Step 3: Uploading resume to Cloudinary...`);
          const uploadResult = await cloudinaryService.uploadResume(
            attachment.content,
            attachment.filename
          );
          logger.info(`Step 3: Resume uploaded successfully - URL: ${uploadResult.url}`);
          ...
          logger.info(`Extracted info: firstName=${firstName}, lastName=${lastName}, email=${candidateEmail}`);
          ...
          logger.info(`Matched to job: ${job.title} (ID: ${job._id})`);
          ...
          stats.errors++;
          logger.error(`Error processing email from ${email.from}:`, error?.message || 'Unknown error');
